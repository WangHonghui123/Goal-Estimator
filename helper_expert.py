...
def expert_find(
    data, num_of_obs, expert_set_train, expert_set_val, step, weighted=False,  gamma=2.0,
):
    ...
    for i in range(num_of_obs):
        ...
        traj_weight = torch.range(1, 2.875, step=0.25)
        ...
        loss_v = criterion(
            ...
            tmp_traj_v.permute(0, 2, 1),
            torch.cat(
                [expert_set_train.obs_traj_v, expert_set_val.obs_traj_v],
                0,
            ).permute(0, 2, 1)
            # * traj_weight,
        )
        ...
        # )
        col_pred_traj = torch.cat(
            [expert_set_train.pred_traj, expert_set_val.pred_traj], 0
        )
        ...
        for k in kmeans.cluster_centers_:
            ...
            test_end = data.pred_traj_gt[:, -1, i]
